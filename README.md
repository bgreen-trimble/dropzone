...
Each method uses a different interface which describes the data available. The code parses that data to find potential images. There may be multiple images in the data. There may be duplicates.

Not all discovered images can be retrieved. If the image identified by a URL, the host may prevent its download by denying a cross-origin resource sharing (CORS) request or authorization error.
...
By convention drag and drop, and clipboard data are identified using MIME types. The data is in the form of key/value pairs. The MIME type is the key.
...
The image MIME type identifies the format of an image file, such as *image/jpeg* for JPEG images, or *image/png* for PNG images.

Normally an image MIME type's data value is a BLOB object (raw data) containing the contents of the image.

An image MIME type only appears when it is copied from the clipboard.
